...
def univariate_tests(dataset, datasetdir, continuous_covs=[],
    ...
    if type(categorical_covs) not in (list, tuple):
    ...
    # stratify = categorical_covs + continuous_covs
    # validation = 50
    ...
    #     if name in continuous_covs:
    #         y[name] = discretizer(y[name].values)
    # splitted = splitter.split(subjects, y)
    # for fold, (train_idx, valid_idx) in enumerate(splitted):
    #     rois_train_idx = rois_idx[train_idx]
    #     clinical_train_idx = clinical_idx[train_idx]
    #     rois_valid_idx = rois_idx[valid_idx]
    #     clinical_valid_idx = clinical_idx[valid_idx]


    # rois_train_data = StandardScaler().fit_transform(rois_data[rois_train_idx])
    # clinical_train_data = StandardScaler().fit_transform(clinical_data[clinical_train_idx])

    # rois_test_data = StandardScaler().fit_transform(rois_data[rois_valid_idx])
    # clinical_test_data = StandardScaler().fit_transform(clinical_data[clinical_valid_idx])
    rois_scaler = StandardScaler()
    clinical_scaler = StandardScaler()
    rois_data = rois_scaler.fit_transform(rois_data[rois_idx])
    clinical_data = clinical_scaler.fit_transform(clinical_data[clinical_idx])
    ...
    for score_idx, score in enumerate(clinical_names):
        ...
        for roi_idx, roi in enumerate(rois_names):
            ...
            model = sm.OLS.from_formula(
                ...
                data=df)
            results = model.fit()
            coefs[score_idx, roi_idx] = results.params["score"]
    ...
    num = 12
    for score in scores:
        score_idx = clinical_names.tolist().index(score)
        # print(score, idx_sign[score_idx].sum())
        for metric in metrics:
            metric_indices = np.array(
                [roi_idx for roi_idx, name in enumerate(rois_names)
                 if metric in name])
            ordered_indices = np.argsort(np.absolute(coefs[score_idx, metric_indices]))[::-1]
            rois_idx = metric_indices[ordered_indices[:num]]
            ...
            values = coefs[score_idx, rois_idx].squeeze()
            # print(len(areas))
            # print(values.shape)
            if rescaled:
                scaling_factors = []
                for roi_idx in rois_idx:
                    scaling_factor = (rois_scaler.scale_[roi_idx] /
                        clinical_scaler.scale_[score_idx])
